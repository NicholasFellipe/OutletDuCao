...
// fim do metodo upload
    ...
    public function Upload($largura, $campo) {
        ...
        // Dados do servidor
        $servidor = 'br976.hostgator.com.br'; // Endereço
        $usuario = 'outletducao@outletducao.com.br'; // Usuário
        $senha = 'outlet'; // Senha
...
// Define variáveis para o envio de arquivo
        $local_arquivo = 'c:/imagemteste.jpg'; // Localização (local)
        $ftp_pasta ='/' ; // Pasta (externa)
        $ftp_arquivo = date('ymdHi').'imagemteste.jpg'.'.jpg'; // Nome do arquivo (externo)
// Envia o arquivo pelo FTP em modo ASCII
      echo  $envio = ftp_put($ftp, $ftp_pasta . $ftp_arquivo, $local_arquivo, FTP_ASCII); // Retorno: true / false
            ...
            if (in_array(strtolower($arquivo_info['extension']), $this->tipos)):
                ...
                echo '<div class="container"><h3 class="alert alert-danger text-center">Arquivo inválido</h3></div>';
                // retorno os dados
                $this->retorno = '<div class="container"><h3 class="alert alert-danger text-center">Tipo de arquivo incorreto</h3></div>';
